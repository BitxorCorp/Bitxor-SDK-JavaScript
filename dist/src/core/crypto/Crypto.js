...
Crypto._encode = (senderPriv, recipientPub, msg, iv) => {
    ...
    const encKey = Buffer.from(utility.bitxorcore_crypto.deriveSharedKey(keyPair.privateKey, Convert_1.Convert.hexToUint8(recipientPub)), 32);
...
Crypto._decode = (recipientPrivate, senderPublic, payload, tagAndIv) => {
    ...
    const encKey = Buffer.from(utility.bitxorcore_crypto.deriveSharedKey(keyPair.privateKey, Convert_1.Convert.hexToUint8(senderPublic)), 32);

...
import { MetadataInfoDTO, MetadataRoutesApi } from 'bitxor-openapi-typescript-fetch-client';
...
export class MetadataHttp extends Http implements MetadataRepository {
    /**
     * @internal
     * Bitxor openapi typescript-node client metadata routes api
     */
    ...
    private toMetadata(metadata: MetadataInfoDTO): Metadata {
        ...
        switch (metadataEntry.metadataType.valueOf()) {
            case MetadataType.Token:
                targetId = new TokenId(metadataEntry.targetId as any);
                break;

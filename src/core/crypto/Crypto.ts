...
export class Crypto {
    ...
    public static _encode = (senderPriv: string, recipientPub: string, msg: string, iv: Uint8Array): string => {
        ...
        const encKey = Buffer.from(utility.bitxorcore_crypto.deriveSharedKey(keyPair.privateKey, convert.hexToUint8(recipientPub)), 32);
    ...
    public static _decode = (recipientPrivate: string, senderPublic: string, payload: Uint8Array, tagAndIv: Uint8Array): string => {
        ...
        const encKey = Buffer.from(utility.bitxorcore_crypto.deriveSharedKey(keyPair.privateKey, convert.hexToUint8(senderPublic)), 32);

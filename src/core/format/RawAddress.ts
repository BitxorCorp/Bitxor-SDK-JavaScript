...
export class RawAddress {
    ...
    public static publicKeyToAddress = (publicKey: Uint8Array, networkType: NetworkType): Uint8Array => {
        ...
        RawArray.copy(decodedAddress, ripemdHash, RawAddress.constants.sizes.ripemd160, 2);

        // step 4: concatenate (3) and the checksum of (3)
        const hash = sha3_256.arrayBuffer(decodedAddress.subarray(0, RawAddress.constants.sizes.ripemd160 + 2));

        RawArray.copy(
            decodedAddress,
            RawArray.uint8View(hash),
            RawAddress.constants.sizes.checksum,
            RawAddress.constants.sizes.ripemd160 + 2,
        );

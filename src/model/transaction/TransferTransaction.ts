...
export class TransferTransaction extends Transaction {
    ...
    public static createFromPayload(payload: string, isEmbedded = false): Transaction | InnerTransaction {
        ...
        const transaction = TransferTransaction.create(
            ...
            builder.getTokens().map((token) => {
                const id = new UInt64(token.tokenId.unresolvedTokenId).toHex();
                return new Token(UnresolvedMapping.toUnresolvedToken(id), new UInt64(token.amount.amount));
            }),
    ...
    protected createBuilder(): TransactionBuilder {
        return new TransferTransactionBuilder(
            ...
            this.sortTokens().map((token) => {
                return new UnresolvedTokenBuilder(new UnresolvedTokenIdDto(token.id.id.toDTO()), new AmountDto(token.amount.toDTO()));
            }),
    ...
    public resolveAliases(statement: Statement, aggregateTransactionIndex = 0): TransferTransaction {
        ...
        return DtoMapping.assign(this, {
            ...
            tokens: this.tokens.map((token) =>
                statement.resolveToken(token, transactionInfo.height.toString(), transactionInfo.index, aggregateTransactionIndex),
            ),
        });
    }

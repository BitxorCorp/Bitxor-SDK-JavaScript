...
import {
    AmountDto,
    EmbeddedTokenMetadataTransactionBuilder,
    EmbeddedTransactionBuilder,
    TimestampDto,
    TokenMetadataTransactionBuilder,
    TransactionBuilder,
 ...
 */
export class TokenMetadataTransaction extends Transaction {
    /**
    ...
    public static create(
        ...
        return new TokenMetadataTransaction(
            networkType,
            TransactionVersion.TOKEN_METADATA,
            deadline,
            maxFee,
            targetAddress,
            scopedMetadataKey,
    ...
    constructor(
         ...
         */
        public readonly targetTokenId: UnresolvedTokenId,
        /**
        ...
        super(TransactionType.TOKEN_METADATA, networkType, version, deadline, maxFee, signature, signer, transactionInfo);
    }
    ...
    public static createFromPayload(payload: string, isEmbedded = false): Transaction | InnerTransaction {
        const builder = isEmbedded
            ? EmbeddedTokenMetadataTransactionBuilder.loadFromBinary(Convert.hexToUint8(payload))
            : TokenMetadataTransactionBuilder.loadFromBinary(Convert.hexToUint8(payload));
        const signerPublicKey = Convert.uint8ToHex(builder.getSignerPublicKey().publicKey);
        const networkType = builder.getNetwork().valueOf();
        const signature = Transaction.getSignatureFromPayload(payload, isEmbedded);
        const transaction = TokenMetadataTransaction.create(
            isEmbedded
                ? Deadline.createEmtpy()
                : Deadline.createFromDTO((builder as TokenMetadataTransactionBuilder).getDeadline().timestamp),
            ...
            UnresolvedMapping.toUnresolvedToken(new UInt64(builder.getTargetTokenId().unresolvedTokenId).toHex()),
            builder.getValueSizeDelta(),
            builder.getValue(),
            networkType,
            isEmbedded ? new UInt64([0, 0]) : new UInt64((builder as TokenMetadataTransactionBuilder).fee.amount),
            signature,
    ...
    protected createBuilder(): TransactionBuilder {
        const transactionBuilder = new TokenMetadataTransactionBuilder(
            this.getSignatureAsBuilder(),
            this.getSignerAsBuilder(),
            this.versionToDTO(),
            this.networkType.valueOf(),
            TransactionType.TOKEN_METADATA.valueOf(),
            ...
            new UnresolvedTokenIdDto(this.targetTokenId.id.toDTO()),
            this.valueSizeDelta,
            this.value,
        );
    ...
    public toEmbeddedTransaction(): EmbeddedTransactionBuilder {
        return new EmbeddedTokenMetadataTransactionBuilder(
            this.getSignerAsBuilder(),
            this.versionToDTO(),
            this.networkType.valueOf(),
            TransactionType.TOKEN_METADATA.valueOf(),
     ...
     * @returns {TokenMetadataTransaction}
     */
    resolveAliases(statement: Statement, aggregateTransactionIndex = 0): TokenMetadataTransaction {
        const transactionInfo = this.checkTransactionHeightAndIndex();
        return DtoMapping.assign(this, {
            targetTokenId: statement.resolveTokenId(
                this.targetTokenId,

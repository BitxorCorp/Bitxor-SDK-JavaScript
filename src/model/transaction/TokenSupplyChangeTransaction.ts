...
import {
    AmountDto,
    EmbeddedTokenSupplyChangeTransactionBuilder,
    EmbeddedTransactionBuilder,
    TimestampDto,
    TokenSupplyChangeTransactionBuilder,
    TransactionBuilder,
 ...
 */
export class TokenSupplyChangeTransaction extends Transaction {
    /**
    ...
    public static create(
        ...
        return new TokenSupplyChangeTransaction(
            networkType,
            TransactionVersion.TOKEN_SUPPLY_CHANGE,
            deadline,
            maxFee,
            tokenId,
            action,
    ...
    constructor(
         ...
         * The supply type.
         */
        public readonly action: TokenSupplyChangeAction,
        /**
        ...
        super(TransactionType.TOKEN_SUPPLY_CHANGE, networkType, version, deadline, maxFee, signature, signer, transactionInfo);
    }
    ...
    public static createFromPayload(payload: string, isEmbedded = false): Transaction | InnerTransaction {
        const builder = isEmbedded
            ? EmbeddedTokenSupplyChangeTransactionBuilder.loadFromBinary(Convert.hexToUint8(payload))
            : TokenSupplyChangeTransactionBuilder.loadFromBinary(Convert.hexToUint8(payload));
        const signerPublicKey = Convert.uint8ToHex(builder.getSignerPublicKey().publicKey);
        const networkType = builder.getNetwork().valueOf();
        const signature = Transaction.getSignatureFromPayload(payload, isEmbedded);
        const transaction = TokenSupplyChangeTransaction.create(
            isEmbedded
                ? Deadline.createEmtpy()
                : Deadline.createFromDTO((builder as TokenSupplyChangeTransactionBuilder).getDeadline().timestamp),
            ...
            networkType,
            isEmbedded ? new UInt64([0, 0]) : new UInt64((builder as TokenSupplyChangeTransactionBuilder).fee.amount),
            signature,
    ...
    protected createBuilder(): TransactionBuilder {
        const transactionBuilder = new TokenSupplyChangeTransactionBuilder(
            this.getSignatureAsBuilder(),
            this.getSignerAsBuilder(),
            this.versionToDTO(),
            this.networkType.valueOf(),
            TransactionType.TOKEN_SUPPLY_CHANGE.valueOf(),
    ...
    public toEmbeddedTransaction(): EmbeddedTransactionBuilder {
        return new EmbeddedTokenSupplyChangeTransactionBuilder(
            this.getSignerAsBuilder(),
            this.versionToDTO(),
            this.networkType.valueOf(),
            TransactionType.TOKEN_SUPPLY_CHANGE.valueOf(),
     ...
     * @returns {TokenSupplyChangeTransaction}
     */
    resolveAliases(statement: Statement, aggregateTransactionIndex = 0): TokenSupplyChangeTransaction {

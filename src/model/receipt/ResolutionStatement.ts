...
import {
    ...
    TokenResolutionEntryBuilder,
    TokenResolutionStatementBuilder,
 ...
 */
export type TokenIdResolutionStatement = ResolutionStatement<UnresolvedTokenId, TokenId>;
 ...
 */
export class ResolutionStatement<U extends UnresolvedAddress | UnresolvedTokenId, R extends Address | TokenId> {
    /**
    ...
    public generateHash(networkType: NetworkType): string {
        const type =
            this.resolutionType === ResolutionType.Address ? ReceiptType.Address_Alias_Resolution : ReceiptType.Token_Alias_Resolution;
        const builder =
            this.resolutionType === ResolutionType.Address
                ...
                : new TokenResolutionStatementBuilder(
                      ReceiptVersion.RESOLUTION_STATEMENT,
                      type.valueOf(),
                      new UnresolvedTokenIdDto(UInt64.fromHex((this.unresolved as UnresolvedTokenId).toHex()).toDTO()),
                      this.resolutionEntries.map(
                          (entry) =>
                              new TokenResolutionEntryBuilder(
                                  new ReceiptSourceBuilder(entry.source.primaryId, entry.source.secondaryId),
                                  (entry.resolved as TokenId).toBuilder(),
                              ),
                      ),
                  );

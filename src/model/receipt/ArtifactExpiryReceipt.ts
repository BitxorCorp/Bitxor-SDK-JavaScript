...
import { NamespaceExpiryReceiptBuilder, TokenExpiryReceiptBuilder } from 'bitxor-catbuffer-typescript';
import { NamespaceId } from '../namespace';
import { TokenId, UnresolvedTokenId } from '../token';
...
export class ArtifactExpiryReceipt extends Receipt {
     ...
     */
    constructor(public readonly artifactId: UnresolvedTokenId, version: ReceiptVersion, type: ReceiptType, size?: number) {
    ...
    public serialize(): Uint8Array {
        if (!this.artifactId.isNamespaceId()) {
            return new TokenExpiryReceiptBuilder(
                ReceiptVersion.ARTIFACT_EXPIRY,
                this.type.valueOf(),
                (this.artifactId as TokenId).toBuilder(),
            ).serialize();
        }

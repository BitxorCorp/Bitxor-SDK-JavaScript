...
export class MetadataTransactionService {
     ...
     */
    public createTokenMetadataTransaction(
        deadline: Deadline,
        networkType: NetworkType,
        targetAddress: Address,
        tokenId: TokenId,
        ...
        sourceAddress: Address,
        maxFee: UInt64,
    ): Observable<TokenMetadataTransaction> {
        return this.metadataRepository
            .search({ targetId: tokenId, scopedMetadataKey: key.toHex(), sourceAddress: sourceAddress, metadataType: MetadataType.Token })
            .pipe(
                map((metadatas: Page<Metadata>) => {
                    if (metadatas.data.length > 0) {
                        const metadata = metadatas.data[0];
                        ...
                        return TokenMetadataTransaction.create(
                            deadline,
                            targetAddress,
                            key,
                            tokenId,

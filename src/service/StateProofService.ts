...
export class StateProofService {
     ...
     * @returns {Observable<StateMerkleProof>}
     */
    public tokenById(tokenId: TokenId): Observable<StateMerkleProof> {
        const tokenRepo = this.repositoryFactory.createTokenRepository();
        return tokenRepo.getToken(tokenId).pipe(
            mergeMap((info) => {
    ...
    public token(info: TokenInfo): Observable<StateMerkleProof> {
        const tokenRepo = this.repositoryFactory.createTokenRepository();
        return tokenRepo.getTokenMerkle(info.id).pipe(
            map((merkle) => {
     ...
     * @returns {Observable<StateMerkleProof>}
     */
    public tokenRestrictionById(compositeHash: string): Observable<StateMerkleProof> {
        const restrictionRepo = this.repositoryFactory.createRestrictionTokenRepository();
        return restrictionRepo.getTokenRestrictions(compositeHash).pipe(mergeMap((info) => this.tokenRestriction(info)));
    }

    public tokenRestriction(info: TokenRestriction): Observable<StateMerkleProof> {
        const restrictionRepo = this.repositoryFactory.createRestrictionTokenRepository();
        return restrictionRepo.getTokenRestrictionsMerkle(info.compositeHash).pipe(map((merkle) => this.toProof(info.serialize(), merkle)));
    }
